...
def dim_reduce(z,method='UMAP',n_neighbor=5,min_dist=0.3):
    
    if method=='UMAP':
        reducer = umap.UMAP(random_state=2022,n_neighbors=n_neighbor,min_dist=min_dist,metric='euclidean') #euclidean manhattan correlation
    else:
        reducer = PCA(n_components=2)

    tiny = reducer.fit_transform(z)
    print("pca.explained_variance_ratio_", reducer.explained_variance_ratio_)
    return tiny
...
def plot_four(DX_batch,TD_batch,model,save_path,epoch,device,idx=0,v=2,s=11,k=40,axis='ax'):
    # im_test = [DX_batch, TD_batch][idx] #torch.Size([64, 1, 64, 64, 64])
    # print("im_test",im_test.shape)
    im_in = [DX_batch.numpy(),TD_batch.numpy()][idx]
    im_in_tensor=torch.FloatTensor(im_in)
    # print("im_in",im_in.shape) #(64, 1, 64, 64, 64)
    # print("m_in_tensor",im_in_tensor.shape)
    encode_s=model.encoder_S(im_in_tensor.to(device))
    encode_z=model.encoder_Z(im_in_tensor.to(device))
    encode_s_2=encode_s[2].cpu().detach().numpy()
    encode_s_v = encode_s[v].cpu().detach().numpy()
    encode_z_v=encode_z[v].cpu().detach().numpy()
    _zeros = np.zeros(encode_s_2.shape)

    cvae_sal_vec = np.hstack((_zeros,encode_s_v))
    cvae_bg_vec = np.hstack((encode_z_v,_zeros))
    
    if idx==1:
        cvae_full_vec = np.hstack((encode_z_v,encode_s_v))
    elif idx==0:
        cvae_full_vec = cvae_bg_vec
    cvae_sal_vec = torch.FloatTensor(cvae_sal_vec)
    cvae_bg_vec=torch.FloatTensor(cvae_bg_vec)
    cvae_full_vec=torch.FloatTensor(cvae_full_vec)
    decoder_sal=model.decoder(cvae_sal_vec.to(device))
    decoder_bg = model.decoder(cvae_bg_vec.to(device))
    decoder_full=model.decoder(cvae_full_vec.to(device))

    decoder_sal=decoder_sal.cpu().detach().numpy()
    decoder_bg=decoder_bg.cpu().detach().numpy()
    decoder_full=decoder_full.cpu().detach().numpy()
    # print("decoder_sal",decoder_sal.shape) #(64, 1, 64, 64, 64)
    # print("decoder_bg",decoder_bg.shape)#(64, 1, 64, 64, 64)
    # print("decoder_full",decoder_full.shape)#(64, 1, 64, 64, 64)
    if axis=='ax':
        plot_im_input = im_in[s,0,:,:,k]
        plot_im_sal = decoder_sal[s,0,:,:,k]
        plot_im_bg = decoder_bg[s,0,:,:,k]
        plot_im_recon = decoder_full[s,0,:,:,k]
    elif axis=='sag':
        plot_im_input = im_in[s,0,k,:,:]
        plot_im_sal = decoder_sal[s,0,k,:,:]
        plot_im_bg = decoder_bg[s,0,k,:,:]
    ...
    plt.subplot(1,4,1)
    plt.imshow(plot_im_input,cmap='gray');plt.xticks([]);plt.yticks([]);plt.title('input')
    plt.subplot(1,4,2)
    plt.imshow(plot_im_recon,cmap='gray');plt.xticks([]);plt.yticks([]);plt.title('reconstruction')
    plt.subplot(1,4,3)
    plt.imshow(plot_im_sal,cmap='gray');plt.xticks([]);plt.yticks([]);plt.title('salient')
    plt.subplot(1,4,4)
    plt.imshow(plot_im_bg,cmap='gray');plt.xticks([]);plt.yticks([]);plt.title('background')
    plt.savefig(os.path.join(save_path,'epoch_{}_input_recon_salient_bg.png'.format(epoch)))
    # plt.show()
...
def plot_trainProgress(loss,im,im1,save_path,epoch):
    ...
    plt.savefig(os.path.join(save_path,"epoch_{}_ori_recon_image.png".format(epoch)))
    # plt.show()
...
def get_triu(inMat):
    ...
    n = inMat.shape[0]
    triu_vec = inMat[np.triu_indices(n=n,k=1)] #返回上三角矩阵的index
    return triu_vec
